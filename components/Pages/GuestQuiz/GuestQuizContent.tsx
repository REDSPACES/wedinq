...
import { isCorrectAnswer } from "../../../lib/constants/quiz-questions";
import {
	registerGuest,
	submitAnswer,
...
export default function GuestQuizContent() {
	...
	}, [session, screenState, sessionStatus]);
		...
		try {
			...
			if (!currentSessionId) {
				...
				if (currentSessionId) {
					...
					alert(
						"セッションが開始されていません。司会者がクイズを開始するまでお待ちください。",
					);
			...
			// Firestoreにニックネームを登録
			const newGuestId = await registerGuest(currentSessionId, nickname.trim());
			setGuestId(newGuestId);
			saveGuestId(newGuestId);
			saveNickname(nickname.trim());
			saveSessionId(currentSessionId);

			console.log("Registered guest:", { guestId: newGuestId, nickname });
	...
	const handleAnswerSubmit = useCallback(async () => {
		...
		try {
			...
			// 正解判定
			const correct = isCorrectAnswer(currentQuestion, selectedChoice);
