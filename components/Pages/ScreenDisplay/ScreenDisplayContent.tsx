...
import { getSessionId } from "../../../lib/utils/session-manager";
...
// セッション状態と問題番号に基づいてスライドインデックスを決定
function getSlideIndexFromSession(
	...
	// playing: 問題番号に応じてスライドを決定
	// 問題1 -> スライド3 (4-question-1.jpg)
	// 問題2 -> スライド5 (6-question-2.jpg)
	// 問題3 -> スライド7 (8-question-3.jpg)
	// 問題4 -> スライド9 (10-question-4.jpg)
	// 問題5 -> スライド11 (12-question-5.jpg)
	const questionSlideIndex = 3 + (currentQuestion - 1) * 2;
	return Math.min(questionSlideIndex, SLIDE_FILENAMES.length - 1);
}
...
export default function ScreenDisplayContent() {
	...
	// セッション状態に応じてスライドを自動切り替え
	useEffect(() => {
		if (!session) return;

		const newSlideIndex = getSlideIndexFromSession(
			session.status,
